...
`:Mop` 以降は、バッファの編集が `mop update` で（**保存前の内容がそのまま**）、カーソル移動が `mop scroll` で送られる。
...
| `g:mop_scroll_delay`   | 60   | カーソル移動を送るまでのデバウンス (ms)                           |
| `g:mop_viewport_ratio` | -1   | 負値ならエディタ側のカーソル位置（画面内の相対位置）をそのまま使う |
