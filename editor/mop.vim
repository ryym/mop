...
"
" Source this file and run :Mop in a Markdown buffer. The preview opens in the
" browser and then follows the buffer: edits are pushed with `mop update`
" (from the buffer, not the file on disk, so unsaved text shows up), and
" cursor movement is pushed with `mop scroll`. :MopClose stops the sync.
"
...
function! s:close_current() abort
  let l:bufnr = bufnr('%')
  if !has_key(s:sessions, l:bufnr)
...
command! -bar Mop call s:open()
command! -bar MopClose call s:close_current()
